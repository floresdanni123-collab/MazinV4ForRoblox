...
local drawingactor = loadstring(downloadFile('newmazin/libraries/drawing.lua'), 'drawing')(...)
...
if not select(1, ...) then
	if run_on_actor and getactors then 
		local oldreload = shared.mazinreload
		mazin.Load = function()
		...
		task.spawn(function()
			repeat task.wait() until not shared.mazin
			local executionString = "loadfile('newmazin/main.lua')("..drawingactor..")"
			...
			if oldreload then 
				executionString = 'shared.mazinreload = true\n'..executionString 
			end
			...
			notif('Mazin', 'Failed to find actor', 10, 'alert')
		end)
	else
		mazin.Load = function()
			notif('Mazin', 'Missing actor functions.', 10, 'alert')
		end
	end
...
local function hookEvent(id, rfunc)
	...
	if not suc then 
		notif('Mazin', 'Failed to hook ('..id..')', 10, 'alert') 
	end
...
run(function()
	...
	until frontlines.ShootFunction or mazin.Loaded == nil
...
run(function()
	...
	GrenadeTP = mazin.Categories.Blatant:CreateModule({
		Name = 'GrenadeTP',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	GunModifications = mazin.Categories.Blatant:CreateModule({
...
run(function()
	...
	GrenadeESP = mazin.Categories.Render:CreateModule({
		Name = 'GrenadeESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NoHurtCam = mazin.Categories.Render:CreateModule({
		Name = 'NoHurtCam',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ThirdPerson = mazin.Categories.Render:CreateModule({
		Name = 'ThirdPerson',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoRespawn = mazin.Categories.Utility:CreateModule({
		Name = 'AutoRespawn',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BulletTracers = mazin.Legit:CreateModule({
		Name = 'BulletTracers',
		Function = function(callback)
			if callback then 
