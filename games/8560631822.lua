...
local tween = mazin.Libraries.tween
local targetinfo = mazin.Libraries.targetinfo
local getfontsize = mazin.Libraries.getfontsize
local getcustomasset = mazin.Libraries.getcustomasset
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('newmazin/assets/new/blur.png')
...
local function isFriend(plr, recolor)
	if mazin.Categories.Friends.Options['Use friends'].Enabled then
		local friend = table.find(mazin.Categories.Friends.ListEnabled, plr.Name) and true
		if recolor then
			friend = friend and mazin.Categories.Friends.Options['Recolor visuals'].Enabled
		end
		return friend
	end
	return nil
...
local function isTarget(plr)
	return table.find(mazin.Categories.Targets.ListEnabled, plr.Name) and true
end
...
local function canClick()
	...
	return (not mazin.gui.ScaledGui.ClickGui.Visible) and (not inputService:GetFocusedTextBox())
end
...
local function notif(...)
	return mazin:CreateNotification(...)
end
...
local function serverHop(pointer, filter)
	visited = shared.mazinserverhoplist and shared.mazinserverhoplist:split('/') or {}
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('Mazin', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('Mazin', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('Mazin', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end

mazin:Clean(lplr.OnTeleport:Connect(function()
	if not tpSwitch then
		tpSwitch = true
		queue_on_teleport("shared.mazinserverhoplist = '"..table.concat(visited, '/').."'\nshared.mazinserverhopprevious = '"..game.JobId.."'")
	end
end))
...
local hash = loadstring(downloadFile('newmazin/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('newmazin/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('newmazin/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
mazin.Libraries.entity = entitylib
mazin.Libraries.whitelist = whitelist
mazin.Libraries.prediction = prediction
mazin.Libraries.hash = hash
mazin.Libraries.auraanims = {
	Normal = {
...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		if mazin.Categories.Main.Options['Teams by server'].Enabled then
	...
	entitylib.getEntityColor = function(ent)
		ent = ent.Player
		if not (ent and mazin.Categories.Main.Options['Use team color'].Enabled) then return end
		if isFriend(ent, true) then
			return Color3.fromHSV(mazin.Categories.Friends.Options['Friends color'].Hue, mazin.Categories.Friends.Options['Friends color'].Sat, mazin.Categories.Friends.Options['Friends color'].Value)
		end
		return tostring(ent.TeamColor) ~= 'White' and ent.TeamColor.Color or nil
	end

	mazin:Clean(function()
		entitylib.kill()
		entitylib = nil
	end)
	mazin:Clean(mazin.Categories.Friends.Update.Event:Connect(function() entitylib.refresh() end))
	mazin:Clean(mazin.Categories.Targets.Update.Event:Connect(function() entitylib.refresh() end))
	mazin:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
	mazin:Clean(workspace:GetPropertyChangedSignal('CurrentCamera'):Connect(function()
...
run(function()
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				olduninject = mazin.Uninject
				mazin.Uninject = function()
					notif('Mazin', 'No escaping the private members :)', 10)
				end
				mazin.Save = function() end
				if joined then
					task.wait(10)
				end
	...
	function whitelist:process(msg, plr)
		...
		if self.localprio > 0 and not self.said[plr.Name] and msg == 'helloimusinginhaler' and plr ~= lplr then
			...
			notif('Mazin', plr.Name..' is using mazin!', 60)
	...
	function whitelist:hook()
		...
		if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
			if exp and exp:WaitForChild('appLayout', 5) then
				mazin:Clean(exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
		...
		if exp then
			...
			if bubblechat then
				mazin:Clean(bubblechat.DescendantAdded:Connect(function(newbubble)
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('newmazin/profiles/whitelist.json') and readfile('newmazin/profiles/whitelist.json') or nil
			end
			local suc, res = pcall(function()
			...
			if not whitelist.connection then
				...
				mazin:Clean(whitelist.connection)
			end
			...
			if entitylib.Running and mazin.Loaded then
				entitylib.refresh()
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				if whitelist.data.Announcement.expiretime > os.time() then
					...
					if table.find(targets, tostring(lplr.UserId)) then
						...
						hint.Text = 'MAZIN ANNOUNCEMENT: '..whitelist.data.Announcement.text
				...
				pcall(function()
					writefile('newmazin/profiles/whitelist.json', whitelist.textdata)
				end)
			end

			if whitelist.data.KillMazin then
				mazin:Uninject()
				return true
			end
			...
			task.spawn(function()
				...
				task.delay(0.6, function()
					local modPrompt = Roact.createElement(auth, {
						style = {},
						screenSize = mazin.gui.AbsoluteSize or Vector2.new(1920, 1080),
						moderationDetails = {
...
run(function()
	...
	AimAssist = mazin.Categories.Combat:CreateModule({
		Name = 'AimAssist',
		Function = function(callback)
			...
			if callback then 
				...
				AimAssist:Clean(runService.RenderStepped:Connect(function(dt)
					...
					if rightClicked and not mazin.gui.ScaledGui.ClickGui.Visible then
	...
	AimAssist:CreateToggle({
		...
		Function = function(callback)
			if callback then
				...
				CircleObject.Position = mazin.gui.AbsoluteSize / 2
...
run(function()
	...
	AutoClicker = mazin.Categories.Combat:CreateModule({
		Name = 'AutoClicker',
		Function = function(callback)
			if callback then
				repeat
					if Mode.Value == 'Tool' then
						...
						if mouse1click and (isrbxactive or iswindowactive)() then
							if not mazin.gui.ScaledGui.ClickGui.Visible then
...
run(function()
	...
	Reach = mazin.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SilentAim = mazin.Categories.Combat:CreateModule({
		Name = 'SilentAim',
		Function = function(callback)
			if CircleObject then
...
run(function()
	...
	TriggerBot = mazin.Categories.Combat:CreateModule({
		Name = 'TriggerBot',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AntiFall = mazin.Categories.Blatant:CreateModule({
		Name = 'AntiFall',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Fly = mazin.Categories.Blatant:CreateModule({
		Name = 'Fly',
		Function = function(callback)
			if Platform then
...
run(function()
	...
	HighJump = mazin.Categories.Blatant:CreateModule({
		Name = 'HighJump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitBoxes = mazin.Categories.Blatant:CreateModule({
		Name = 'HitBoxes',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Invisible = mazin.Categories.Blatant:CreateModule({
		Name = 'Invisible',
		Function = function(callback)
			if callback then
				if not proper then
...
run(function()
	...
	Killaura = mazin.Categories.Blatant:CreateModule({
		Name = 'Killaura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	LongJump = mazin.Categories.Blatant:CreateModule({
		Name = 'LongJump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MouseTP = mazin.Categories.Blatant:CreateModule({
		Name = 'MouseTP',
		Function = function(callback)
			if callback then
				local position
...
run(function()
	...
	Phase = mazin.Categories.Blatant:CreateModule({
		Name = 'Phase',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Speed = mazin.Categories.Blatant:CreateModule({
		Name = 'Speed',
		Function = function(callback)
...
run(function()
	...
	Spider = mazin.Categories.Blatant:CreateModule({
		Name = 'Spider',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SpinBot = mazin.Categories.Blatant:CreateModule({
		Name = 'SpinBot',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Swim = mazin.Categories.Blatant:CreateModule({
		Name = 'Swim',
		Function = function(callback)
			if callback then
...
run(function()
	...
	TargetStrafe = mazin.Categories.Blatant:CreateModule({
		Name = 'TargetStrafe',
		Function = function(callback)
			if callback then
				if not module then
				...
				local flymod, ang, oldent = mazin.Modules.Fly or {Enabled = false}
...
run(function()
	...
	Timer = mazin.Categories.Blatant:CreateModule({
		Name = 'Timer',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function Added(ent)
		...
		EntityArrow.Image = getcustomasset('newmazin/assets/new/arrowmodule.png')
	...
	Arrows = mazin.Categories.Render:CreateModule({
		Name = 'Arrows',
		Function = function(callback)
			if callback then
				...
				Arrows:Clean(mazin.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Chams = mazin.Categories.Render:CreateModule({
		Name = 'Chams',
		Function = function(callback)
			if callback then
				...
				Chams:Clean(mazin.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	ESP = mazin.Categories.Render:CreateModule({
		Name = 'ESP',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					ESP:Clean(mazin.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	GamingChair = mazin.Categories.Render:CreateModule({
		Name = 'GamingChair',
		Function = function(callback)
			if callback then
				if mazin.ThreadFix then
				...
				--movingsound.SoundId = downloadMazinAsset('mazin/assets/ChairRolling.mp3')
				movingsound.Volume = 0.4
				movingsound.Looped = true
				movingsound.Parent = workspace
				flyingsound = Instance.new('Sound')
				--flyingsound.SoundId = downloadMazinAsset('mazin/assets/ChairFlying.mp3')
				...
				repeat
					if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 then
						...
						local flying = mazin.Modules.Fly and mazin.Modules.Fly.Enabled or mazin.Modules.LongJump and mazin.Modules.LongJump.Enabled or mazin.Modules.InfiniteFly and mazin.Modules.InfiniteFly.Enabled
...
run(function()
	...
	Health = mazin.Categories.Render:CreateModule({
		Name = 'Health',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NameTags = mazin.Categories.Render:CreateModule({
		Name = 'NameTags',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					NameTags:Clean(mazin.Categories.Friends.ColorUpdate.Event:Connect(function()
						ColorFunc[methodused](Color.Hue, Color.Sat, Color.Value)
					end))
				end
...
run(function()
	...
	PlayerModel = mazin.Categories.Render:CreateModule({
		Name = 'PlayerModel',
		Function = function(callback)
			if callback then 
...
run(function()
	...
	Radar = mazin:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('newmazin/assets/new/radaricon.png'),
		...
		Function = function(callback)
			if callback then
				...
				Radar:Clean(mazin.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Search = mazin.Categories.Render:CreateModule({
		Name = 'Search',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SessionInfo = mazin:CreateOverlay({
		Name = 'Session Info',
			...
			if callback then
				...
				SessionInfo:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
					if not teleportedServers then
						...
						queue_on_teleport("shared.mazinsessioninfo = '"..httpService:JSONEncode(mazin.Libraries.sessioninfo.Objects).."'")
					end
				end))
	
				if shared.mazinsessioninfo then
					for i, v in httpService:JSONDecode(shared.mazinsessioninfo) do
						if mazin.Libraries.sessioninfo.Objects[i] and v.Saved then
							mazin.Libraries.sessioninfo.Objects[i].Value = v.Value
						end
					end
				end
	
				repeat
					if mazin.Libraries.sessioninfo then
						local stuff = {''}
						if Title.Enabled then
							stuff[1] = TitleOffset.Enabled and '<b>Session Info</b>\n<font size="4"> </font>' or '<b>Session Info</b>'
						end
						for i, v in mazin.Libraries.sessioninfo.Objects do
	...
	mazin:Clean(SessionInfo.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
		if mazin.ThreadFix then 
			setthreadidentity(8) 
		end
		local newside = SessionInfo.Children.AbsolutePosition.X > (mazin.gui.AbsoluteSize.X / 2)
	...
	mazin.Libraries.sessioninfo:AddItem('Time Played', os.clock(), function(value) 
...
run(function()
	...
	local function Loop()
		local screenSize = mazin.gui.AbsoluteSize
	...
	Tracers = mazin.Categories.Render:CreateModule({
		Name = 'Tracers',
		Function = function(callback)
			if callback then
				...
				Tracers:Clean(mazin.Categories.Friends.ColorUpdate.Event:Connect(function()
					ColorFunc(Color.Hue, Color.Sat, Color.Value)
				end))
...
run(function()
	...
	WaypointFolder.Parent = mazin.gui
	
	Waypoints = mazin.Categories.Render:CreateModule({
		Name = 'Waypoints',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AnimationPlayer = mazin.Categories.Utility:CreateModule({
...
run(function()
	...
	AntiRagdoll = mazin.Categories.Utility:CreateModule({
		Name = 'AntiRagdoll',
		Function = function(callback)
			if entitylib.isAlive then
...
run(function()
	...
	AutoRejoin = mazin.Categories.Utility:CreateModule({
		Name = 'AutoRejoin',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	Blink = mazin.Categories.Utility:CreateModule({
		Name = 'Blink',
		Function = function(callback)
			if callback then
				local teleported
...
run(function()
	...
	ChatSpammer = mazin.Categories.Utility:CreateModule({
		Name = 'ChatSpammer',
		Function = function(callback)
			if callback then
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	Disabler = mazin.Categories.Utility:CreateModule({
		Name = 'Disabler',
		Function = function(callback)
			if callback then
...
run(function()
	mazin.Categories.Utility:CreateModule({
		Name = 'Panic',
		Function = function(callback)
			if callback then
				for _, v in mazin.Modules do
					if v.Enabled then
						v:Toggle()
					end
				end
...
run(function()
	...
	Rejoin = mazin.Categories.Utility:CreateModule({
		Name = 'Rejoin',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ServerHop = mazin.Categories.Utility:CreateModule({
		Name = 'ServerHop',
		Function = function(callback)
			if callback then
				ServerHop:Toggle()
	...
	ServerHop:CreateButton({
		...
		Function = function()
			notif('ServerHop', shared.mazinserverhopprevious and 'Rejoining previous server...' or 'Cannot find previous server', 5)
			if shared.mazinserverhopprevious then
				teleportService:TeleportToPlaceInstance(game.PlaceId, shared.mazinserverhopprevious)
			end
		end
	})
end)
...
run(function()
	...
	local function playerAdded(plr)
		if not mazin.Loaded then 
			repeat task.wait() until mazin.Loaded 
		end
		...
		if user or getRole(plr, tonumber(Group.Value) or 0) >= (tonumber(Role.Value) or 1) then
			...
			elseif Mode.Value == 'Profile' then
				mazin.Save = function() end
				if mazin.Profile ~= Profile.Value then
					mazin.Profile = Profile.Value
					mazin:Load(true, Profile.Value)
				end
	...
	StaffDetector = mazin.Categories.Utility:CreateModule({
		Name = 'StaffDetector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	mazin.Categories.World:CreateModule({
		Name = 'Anti-AFK',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Freecam = mazin.Categories.World:CreateModule({
		Name = 'Freecam',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Gravity = mazin.Categories.World:CreateModule({
		Name = 'Gravity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Parkour = mazin.Categories.World:CreateModule({
		Name = 'Parkour',
		Function = function(callback)
			if callback then 
				local oldfloor
...
run(function()
	...
	Xray = mazin.Categories.World:CreateModule({
		Name = 'Xray',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MurderMystery = mazin.Categories.Minigames:CreateModule({
		Name = 'MurderMystery',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Atmosphere = mazin.Legit:CreateModule({
		Name = 'Atmosphere',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Breadcrumbs = mazin.Legit:CreateModule({
		Name = 'Breadcrumbs',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ChinaHat = mazin.Legit:CreateModule({
		Name = 'China Hat',
		Function = function(callback)
			if callback then
				if mazin.ThreadFix then
...
run(function()
	...
	Clock = mazin.Legit:CreateModule({
		Name = 'Clock',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Disguise = mazin.Legit:CreateModule({
		Name = 'Disguise',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Keystrokes = mazin.Legit:CreateModule({
		Name = 'Keystrokes',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Memory = mazin.Legit:CreateModule({
		Name = 'Memory',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SongBeats = mazin.Legit:CreateModule({
		Name = 'Song Beats',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Speedmeter = mazin.Legit:CreateModule({
		Name = 'Speedmeter',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	TimeChanger = mazin.Legit:CreateModule({
		Name = 'Time Changer',
		Function = function(callback)
			if callback then
