local loadstring = function(...)
	...
	if err and mazin then mazin:CreateNotification('Mazin', 'Failed to load : '..err, 30, 'alert') end
	return res
end
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/floresdanni123-collab/MazinV4ForRoblox/'..readfile('newmazin/profiles/commit.txt')..'/'..select(1, path:gsub('newmazin/', '')), true) end)
		if not suc or res == '404: Not Found' then error(res) end
		if path:find('.lua') then res = '--This watermark is used to delete the file if its cached, remove it to make the file persist after mazin updates.\n'..res end
		writefile(path, res)
	end
...
local vm = loadstring(downloadFile('newmazin/libraries/vm.lua'), 'vm')()
...
run(function()
	...
	if not jb.VehicleController.toggleLocalLocked or not jb.VehicleController.NitroShopVisible then
		repeat task.wait() until (jb.VehicleController.toggleLocalLocked and jb.VehicleController.NitroShopVisible) or mazin.Loaded == nil
	...
	function jb:FireServer(id, ...)
		if not remotes[id] then
			notif('Mazin', 'Failed to find remote ('..id..')', 10, 'alert')
			return
		end
...
run(function()
	...
	Wallbang = mazin.Categories.Combat:CreateModule({
		Name = 'Wallbang',
		Function = function(callback)
			if callback then
				local hook
...
run(function()
	...
	AutoArrest = mazin.Categories.Blatant:CreateModule({
		Name = 'AutoArrest',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoPop = mazin.Categories.Blatant:CreateModule({
		Name = 'AutoPop',
		Function = function(callback)
			if callback then
				task.spawn(function()
...
run(function()
	...
	Punch = mazin.Categories.Blatant:CreateModule({
		Name = 'AutoPunch',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoTaze = mazin.Categories.Blatant:CreateModule({
		Name = 'AutoTaze',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	LazerGodmode = mazin.Categories.Blatant:CreateModule({Name = 'LazerGodmode'})
end)
...
run(function()
	...
	InfiniteNitro = mazin.Categories.Utility:CreateModule({
		Name = 'InfiniteNitro',
		Function = function(callback)
			if callback then
