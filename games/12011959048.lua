...
mazin.Place = 11630038968
if isfile('newmazin/games/'..mazin.Place..'.lua') then
	loadstring(readfile('newmazin/games/'..mazin.Place..'.lua'), 'bridge duel')()
else
	if not shared.MazinDeveloper then
		local suc, res = pcall(function() return 
			game:HttpGet('https://raw.githubusercontent.com/floresdanni123-collab/MazinV4ForRoblox/'..readfile('newmazin/profiles/commit.txt')..'/games/'..mazin.Place..'.lua', true) 
		end)
		if suc and res ~= '404: Not Found' then
			loadstring(downloadFile('newmazin/games/'..mazin.Place..'.lua'), 'bridge duel')()
		end
	end
end
