...
run(function()
	...
	local function updateStore(newStore, oldStore)
		if newStore.GameCurrency ~= oldStore.GameCurrency then 
			mazinEvents.CurrencyChange:Fire(table.clone(newStore.GameCurrency.Quantities)) 
		end
	...
	mazin:Clean(workspace.BlockContainer.DescendantAdded:Connect(function(v)
		parsePositions(v, function(pos) 
			store.blocks[pos] = v 
		end)
	end))
	mazin:Clean(workspace.BlockContainer.DescendantRemoving:Connect(function(v)
		parsePositions(v, function(pos) 
...
run(function()
	...
	InvMove = mazin.Categories.Blatant:CreateModule({
		Name = 'InvMove',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local anims, armC0 = mazin.Libraries.auraanims
		...
		Function = function(callback)
			if callback then
				...
				repeat
					...
					if Attacking and mazin.ThreadFix then
						setthreadidentity(8)
					end
...
run(function()
	...
	NoFall = mazin.Categories.Blatant:CreateModule({
		Name = 'NoFall',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoBuy = mazin.Categories.Inventory:CreateModule({
		...
		Function = function(callback)
			if callback then
				AutoBuy:Clean(mazinEvents.CurrencyChange.Event:Connect(buyCheck))
	...
	Viewmodel = mazin.Legit:CreateModule({
		...
		Function = function(callback)
			if callback then 
				...
				mazin:Clean(ViewmodelMotor)
				mazin:Clean(runService.RenderStepped:Connect(function()
				...
				mazin:Clean(entitylib.Events.LocalAdded:Connect(newCharacter))
				if entitylib.isAlive then 
