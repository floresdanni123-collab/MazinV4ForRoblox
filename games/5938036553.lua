...
local mazinEvents = setmetatable({}, {
...
local entitylib = mazin.Libraries.entity
local targetinfo = mazin.Libraries.targetinfo
local sessioninfo = mazin.Libraries.sessioninfo
local uipallet = mazin.Libraries.uipallet
local tween = mazin.Libraries.tween
local color = mazin.Libraries.color
local whitelist = mazin.Libraries.whitelist
local prediction = mazin.Libraries.prediction
...
local function hotbarSwitch(slot)
	if slot and store.inventory.hotbarSlot ~= slot then
		...
		mazinEvents.InventoryChanged.Event:Wait()
		return true
	end
	return false
end
			...
			if hum and humrootpart then
				...
				if plr == lplr then
					...
					table.insert(entitylib.Connections, char.AttributeChanged:Connect(function(attr)
						mazinEvents.AttributeChanged:Fire(attr)
					end))
				else
...
run(function()
	...
	for i, v in remoteNames do
		...
		if remote == '' then
			notif('Mazin', 'Failed to grab remote ('..i..')', 10, 'alert')
		end
		remotes[i] = remote
	end
			...
			return {
				...
				SendToServer = function(_, knockback)
					...
					if damage.knockbackId and (not damage.knockbackMultiplier or not damage.knockbackMultiplier.disabled) then
						...
						mazinEvents.KnockbackReceived:Fire()
					end
				end
			}
	...
	local function updateStore(new, old)
		...
		if new.Inventory ~= old.Inventory then
			...
			if newinv ~= oldinv then
				mazinEvents.InventoryChanged:Fire()
			end

			if newinv.inventory.items ~= oldinv.inventory.items then
				mazinEvents.InventoryAmountChanged:Fire()
	...
	for _, event in {'MatchEndEvent', 'EntityDeathEvent', 'EntityDamageEvent', 'BedwarsBedBreak', 'BalloonPopped', 'AngelProgress', 'GrapplingHookFunctions'} do
		if not mazin.Connections then return end
		bedwars.Client:WaitFor(event):andThen(function(connection)
			mazin:Clean(connection:Connect(function(...)
				mazinEvents[event]:Fire(...)
			end))
		end)
	end
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		bedwars.ClientDamageBlock:WaitFor(event):andThen(function(connection)
			if not mazin.Connections then return end
			mazin:Clean(connection:Connect(function(data)
				for i, v in cache do
				...
				mazinEvents[event]:Fire(data)
			end))
		end)
	end

	mazin:Clean(mazinEvents.KnockbackReceived.Event:Connect(function()
	...
	task.spawn(function()
		pcall(function()
			repeat task.wait() until store.matchState ~= 0 or mazin.Loaded == nil
			if mazin.Loaded == nil then return end
			mapname = workspace:WaitForChild('Map', 5):WaitForChild('Worlds', 5):GetChildren()[1].Name
			mapname = string.gsub(string.split(mapname, '_')[2] or mapname, '-', '') or 'Blank'
		end)
	end)

	mazin:Clean(mazinEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
		if bedTable.player and bedTable.player.UserId == lplr.UserId then
			beds:Increment()
		end
	end))

	mazin:Clean(mazinEvents.MatchEndEvent.Event:Connect(function(winTable)
		if (bedwars.Store:getState().Game.myTeam or {}).id == winTable.winningTeamId or lplr.Neutral then
			wins:Increment()
		end
	end))

	mazin:Clean(mazinEvents.EntityDeathEvent.Event:Connect(function(deathTable)
	...
	pcall(function()
		if getthreadidentity and setthreadidentity then
			...
			task.spawn(function()
				repeat
					task.wait(0.1)
				until mazin.Loaded == nil or bedwars.AppController:isAppOpen('BedwarsItemShopApp')
				store.shopLoaded = true
			end)
		end
	end)
...
run(function()
	...
	mazin.Categories.Combat:CreateModule({
		Name = 'NoClickDelay',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Sprint = mazin.Categories.Combat:CreateModule({
		Name = 'Sprint',
		Function = function(callback)
			if callback then
				if inputService.TouchEnabled then 
...
run(function()
	...
	Velocity = mazin.Categories.Combat:CreateModule({
		Name = 'Velocity',
		Function = function(callback)
			if callback then
							...
							if Mode.Value == 'Normal' then
								...
								if top then
									...
									connection = runService.PreSimulation:Connect(function()
										if mazin.Modules.Fly.Enabled or mazin.Modules.InfiniteFly.Enabled or mazin.Modules.LongJump.Enabled then
	...
	Fly = mazin.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			if callback then
				...
				Fly:Clean(mazinEvents.AttributeChanged.Event:Connect(function(changed)
...
run(function()
	...
	InfiniteFly = mazin.Categories.Blatant:CreateModule({
		Name = 'InfiniteFly',
		Function = function(callback)
			frictionTable.InfiniteFly = callback or nil
			updateVelocity()
			if callback then
				if mazin.Modules.Invisible and mazin.Modules.Invisible.Enabled then
					mazin.Modules.Invisible:Toggle()
...
run(function()
	mazin.Categories.Blatant:CreateModule({
		Name = 'KeepSprint',
		Function = function(callback)
...
run(function()
	...
	local anims, AnimDelay, AnimTween, armC0 = mazin.Libraries.auraanims, tick()
...
run(function()
	...
	LongJump = mazin.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			updateVelocity()
			if callback then
				LongJump:Clean(mazinEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				...
				LongJump:Clean(mazinEvents.GrapplingHookFunctions.Event:Connect(function(dataTable)
...
run(function()
	...
	local ProjectileAimbot = mazin.Categories.Blatant:CreateModule({
...
run(function()
	...
	ProjectileAura = mazin.Categories.Blatant:CreateModule({
		Name = 'ProjectileAura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	BedESP = mazin.Categories.Render:CreateModule({
		Name = 'BedESP',
		Function = function(callback)
			if callback then
	...
	Health = mazin.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				Health:Clean(label)
				Health:Clean(mazinEvents.AttributeChanged.Event:Connect(function()
					label.Text = entitylib.isAlive and math.round(lplr.Character:GetAttribute('Health'))..' ❤️' or ''
					label.TextColor3 = entitylib.isAlive and Color3.fromHSV((lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) / 2.8, 0.86, 1) or Color3.new()
				end))
			end
...
run(function()
	...
	KitESP = mazin.Categories.Render:CreateModule({
		Name = 'KitESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	StorageESP = mazin.Categories.Render:CreateModule({
		Name = 'StorageESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBalloon = mazin.Categories.Utility:CreateModule({
		Name = 'AutoBalloon',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoKit = mazin.Categories.Utility:CreateModule({
		Name = 'AutoKit',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPlay = mazin.Categories.Utility:CreateModule({
		Name = 'AutoPlay',
		Function = function(callback)
			if callback then
				AutoPlay:Clean(mazinEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill and deathTable.entityInstance == lplr.Character and isEveryoneDead() and store.matchState ~= 2 then
						joinQueue()
					end
				end))
				AutoPlay:Clean(mazinEvents.MatchEndEvent.Event:Connect(joinQueue))
			end
		end,
...
run(function()
	...
	AutoToxic = mazin.Categories.Utility:CreateModule({
		Name = 'AutoToxic',
		Function = function(callback)
			if callback then
				AutoToxic:Clean(mazinEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
				...
				AutoToxic:Clean(mazinEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				AutoToxic:Clean(mazinEvents.MatchEndEvent.Event:Connect(function(winstuff)
					if GG.Enabled then
						if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	AutoVoidDrop = mazin.Categories.Utility:CreateModule({
		Name = 'AutoVoidDrop',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MissileTP = mazin.Categories.Utility:CreateModule({
		Name = 'MissileTP',
		Function = function(callback)
			if callback then
				MissileTP:Toggle()
...
run(function()
	...
	PickupRange = mazin.Categories.Utility:CreateModule({
		Name = 'PickupRange',
		Function = function(callback)
			if callback then
...
run(function()
	...
	RavenTP = mazin.Categories.Utility:CreateModule({
		Name = 'RavenTP',
		Function = function(callback)
			if callback then
				RavenTP:Toggle()
...
run(function()
	...
	Scaffold = mazin.Categories.Utility:CreateModule({
		Name = 'Scaffold',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	local function staffFunction(plr, checktype)
		if not mazin.Loaded then repeat task.wait() until mazin.Loaded end
	...
	local function playerAdded(plr)
		...
		if getRole(plr, 5774246) >= 100 then
			...
			if table.find(blacklistedclans, plr:GetAttribute('ClanTag')) and mazin.Loaded then
				connection:Disconnect()
	...
	task.spawn(function()
		repeat task.wait(1) until mazin.Loaded or mazin.Loaded == nil
		if mazin.Loaded and not StaffDetector.Enabled then
...
run(function()
	StoreDamage = mazin.Categories.Utility:CreateModule({
		Name = 'StoreDamage',
...
run(function()
	TrapDisabler = mazin.Categories.Utility:CreateModule({
		Name = 'TrapDisabler',
...
run(function()
	...
	AutoTool = mazin.Categories.World:CreateModule({
		Name = 'AutoTool',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BedProtector = mazin.Categories.World:CreateModule({
		Name = 'BedProtector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ChestSteal = mazin.Categories.World:CreateModule({
		Name = 'ChestSteal',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Schematica = mazin.Categories.World:CreateModule({
		Name = 'Schematica',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBuy = mazin.Categories.Inventory:CreateModule({
		Name = 'AutoBuy',
		Function = function(callback)
			if callback then
				repeat task.wait() until store.queueType ~= 'bedwars_test'
				...
				local lastupgrades
				AutoBuy:Clean(mazinEvents.InventoryAmountChanged.Event:Connect(function()
...
run(function()
	...
	AutoConsume = mazin.Categories.Inventory:CreateModule({
		Name = 'AutoConsume',
		Function = function(callback)
			if callback then
				AutoConsume:Clean(mazinEvents.InventoryAmountChanged.Event:Connect(consumeCheck))
				AutoConsume:Clean(mazinEvents.AttributeChanged.Event:Connect(function(attribute)
...
run(function()
	...
	local function CreateWindow(self)
		...
		window.Parent = mazin.gui.ScaledGui
		...
		close.MouseButton1Click:Connect(function()
			window.Visible = false
			mazin.gui.ScaledGui.ClickGui.Visible = true
		end)
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			children.CanvasSize = UDim2.fromOffset(0, windowlist.AbsoluteContentSize.Y / mazin.guiscale.Scale)
		end)
		table.insert(mazin.Windows, window)
	...
	mazin.Components.HotbarList = function(optionsettings, children, api)
		if mazin.ThreadFix then
			setthreadidentity(8)
		end
		local optionapi = {
		...
		textbuttonicon.Image = getcustomasset('newmazin/assets/new/add.png')
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			hotbarlist.Size = UDim2.fromOffset(220, math.min(43 + windowlist.AbsoluteContentSize.Y / mazin.guiscale.Scale, 603))
		end)
		...
		function optionapi:AddHotbar(data)
			...
			hotbar.MouseButton1Click:Connect(function()
				...
				if ind == optionapi.Selected then
					mazin.gui.ScaledGui.ClickGui.Visible = false
	...
	AutoHotbar = mazin.Categories.Inventory:CreateModule({
		Name = 'AutoHotbar',
		Function = function(callback)
			if callback then
				...
				AutoHotbar:Clean(mazinEvents.InventoryAmountChanged.Event:Connect(sortCallback))
			end
		end,
...
run(function()
	...
	local FastConsume = mazin.Categories.Inventory:CreateModule({
		Name = 'FastConsume',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FastDrop = mazin.Categories.Inventory:CreateModule({
		Name = 'FastDrop',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	BedPlates = mazin.Categories.Minigames:CreateModule({
		Name = 'BedPlates',
		Function = function(callback)
			if callback then
				for _, v in collectionService:GetTagged('bed') do 
				...
				BedPlates:Clean(mazinEvents.PlaceBlockEvent.Event:Connect(refreshNear))
				BedPlates:Clean(mazinEvents.BreakBlockEvent.Event:Connect(refreshNear))
...
run(function()
			...
			local mounted = bedwars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						BackgroundTransparency = 1,
						Image = getcustomasset('newmazin/assets/new/blur.png'),
	...
	Breaker = mazin.Categories.Minigames:CreateModule({
		Name = 'Breaker',
		Function = function(callback)
			if callback then
				for _ = 1, 30 do
...
run(function()
	...
	local Crosshair = mazin.Legit:CreateModule({
		Name = 'Crosshair',
		Function = function(callback)
			if callback then 
...
run(function()
	...
	DamageIndicator = mazin.Legit:CreateModule({
...
run(function()
	...
	FPSBoost = mazin.Legit:CreateModule({
		Name = 'FPS Boost',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitColor = mazin.Legit:CreateModule({
		Name = 'Hit Color',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	Interface = mazin.Legit:CreateModule({
		Name = 'Interface',
		Function = function(callback)
...
run(function()
	...
	KillEffect = mazin.Legit:CreateModule({
		Name = 'Kill Effect',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ReachDisplay = mazin.Legit:CreateModule({
		Name = 'Reach Display',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SoundChanger = mazin.Legit:CreateModule({
		Name = 'SoundChanger',
		Function = function(callback)
			if callback then 
...
run(function()
	...
	UICleanup = mazin.Legit:CreateModule({
		Name = 'UI Cleanup',
		Function = function(callback)
			for i, v in (callback and new or old) do
				for i2, v2 in v do 
...
run(function()
	...
	Viewmodel = mazin.Legit:CreateModule({
		Name = 'Viewmodel',
		Function = function(callback)
